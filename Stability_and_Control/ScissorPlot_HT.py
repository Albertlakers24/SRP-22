...
from Constants.AircraftGeometry import S_w, Aw, c_mac_w,c_rw, Sweep_quarterchordw, Sweep_halfchordw, SweepLE, d_f_outer, SwfS, c_accent_c, b_flap, bw
from Constants.Empennage_LandingGear import A_h, lh, Vh_V
...
from Constants.Aerodynamics import Cm0_airfoil, CL0_Land, DeltaCLflaps, CL_DesCruise,CL_Alpha_Wing, CL_Max_Clean, CL_MaxLand
from Constants.MissionInputs import M_cruise, ISA_calculator, h_cruise, dt_cruise, V_approach, g
...
def CL_alpha_Ah():
    ...
    """
    Snet = Sw - cr*bf
    C_L_alpha_Ah = (CL_Alpha_Wing * (1 + (2.15 * (bf / b))) * (Snet / S_w)) + ((np.pi / 2) * (bf ** 2 / S))
    return C_L_alpha_Ah
...
def AC_location():
    ...
    :return: x_ac (MAC)
    """
    x_ac_f1 = -((1.8 * bf * hf * l_fn) / (CL_alpha_Ah() * Sw * c_mac_w))
    x_ac_f2 = ((0.273 * bf * cg * (b - bf)) / ((1 + taper) * c_mac_w ** 2 * (b + 2.15 * bf))) * np.tan(lambda_quarterchord)
...
def flapcont(mu1, mu2, mu3):
    flap_cont_quarter = mu2 * (-mu1 * DeltaClmax * c_accent_c - (CL_MaxLand + DeltaClmax * (1 - SwfS)) * (1 / 8) * c_accent_c*(c_accent_c - 1)) + 0.7 * (A / (1 + 2 / A)) * mu3 * DeltaClmax * np.tan(lambda_quarterchord)
...
def C_m_AC(mu1, mu2, mu3):
    ...
    :return:Cmac (-)
    """
    C_m_acw = Cm0_airfoil* (A*(np.cos(SweepLE)**2))/(A+2*np.cos(SweepLE))
    fus_cont = -1.8 * (1 - 2.5*bf/l_f)*(np.pi * bf * hf * l_f * CL0_Land)/(4*Sw*c_bar * CL_alpha_Ah())
    nac_cont = -0.05
...
def C_L_Ah():
    ...
    C_L_Ah = 2*W_landing/(rho_land*Sw*V_approach**2)
    return C_L_Ah
