...
from Constants.AircraftGeometry import S_w, c_mac_w, taperw, l_f, l_cp, l_cylindrical, l_tc, bw, delta_flap
...
from Constants.Empennage_LandingGear import taperh, taperv, xc_m_tail, tc_tail, cmac_v, cmac_h, Sh, Sv
...
sweep_m_Htail = Calculate_wingsweep(6.14, 0,  xc_m_tail, taperh)  # Sweep at (x/c)_m
sweep_m_Vtail = Calculate_wingsweep(26, 0, xc_m_tail, taperv)  # Sweep at (x/c)_m
...
Swet_wing = 1.07* 2 * (S_w - 2*4.68322)       # TODO: If low wing - change to exposed area!
...
# TODO: If low wing, change interference! and depending on bulge!
...
FF_Htail_CR = Calculate_FF_aerocomp(tc_tail, xc_m_tail, sweep_m_Htail, M_cruise)
FF_Vtail_CR = Calculate_FF_aerocomp(tc_tail, xc_m_tail, sweep_m_Vtail, M_cruise)
...
FF_Htail_TO = Calculate_FF_aerocomp(tc_tail, xc_m_tail, sweep_m_Htail, Mach_TO)
FF_Vtail_TO = Calculate_FF_aerocomp(tc_tail, xc_m_tail, sweep_m_Vtail, Mach_TO)
...
FF_Htail_LD = Calculate_FF_aerocomp(tc_tail, xc_m_tail, sweep_m_Htail, Mach_land)
FF_Vtail_LD = Calculate_FF_aerocomp(tc_tail, xc_m_tail, sweep_m_Vtail, Mach_land)
...
Cd_flap_TO_plain = Calculate_cd_flap(0, delta_f_TO)
Cd_flap_TO_slotted= Calculate_cd_flap(1, delta_f_TO)

Cd_flap_LD_plain = Calculate_cd_flap(0, delta_f_LD)
Cd_flap_LD_slotted= Calculate_cd_flap(1, delta_f_LD)
